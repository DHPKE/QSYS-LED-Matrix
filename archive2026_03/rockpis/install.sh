...
echo "[8/8] Disabling UART0 serial console (pins 8 and 10 needed for HUB75)..."

# Method A — systemd (runtime, takes effect immediately):
sudo systemctl disable --now serial-getty@ttyS0 2>/dev/null || true
sudo systemctl disable --now serial-getty@ttyFIQ0 2>/dev/null || true
...
if [ -f /boot/armbianEnv.txt ]; then
  if grep -q "console=ttyS0" /boot/armbianEnv.txt; then
    sudo sed -i 's/console=ttyS0,[0-9]*//' /boot/armbianEnv.txt
    sudo sed -i 's/  */ /g; s/^ //; s/ $//' /boot/armbianEnv.txt
...
if grep -q "overlays=.*uart" /boot/armbianEnv.txt 2>/dev/null; then
  sudo sed -i 's/uart[0-9]\{1,2\}[a-z]*//' /boot/armbianEnv.txt
...
echo "   console disable. The service is already running, but after"
